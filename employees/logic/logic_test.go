...
func (l *logicTest) TestChanges(t *testing.T) {
	...
	changesRead, err := l.changesClient.ChangesRead(ctx, changesdata.ChangeSearch{
		DataIds:      []string{employeeId},
		Types:        []string{data.ChangeTypeEmployee},
		ServiceNames: []string{data.ServiceName},
		Actions:      []string{data.ChangeActionCreate},
	})
	assert.Nil(t, err)
	...
	assert.Equal(t, data.ChangeActionCreate, changesRead[0].DataAction)
	assert.Equal(t, data.ServiceName, changesRead[0].DataServiceName)
	assert.Equal(t, data.ChangeTypeEmployee, changesRead[0].DataType)
	...
	changesRead, err = l.changesClient.ChangesRead(ctx, changesdata.ChangeSearch{
		...
		Types:        []string{data.ChangeTypeEmployee},
		ServiceNames: []string{data.ServiceName},
		Actions:      []string{data.ChangeActionUpdate},
	})
	assert.Nil(t, err)
	assert.NotNil(t, changesRead)
	assert.Len(t, changesRead, 1)
	...
	assert.Equal(t, data.ChangeActionUpdate, changesRead[0].DataAction)
	assert.Equal(t, data.ServiceName, changesRead[0].DataServiceName)
	assert.Equal(t, data.ChangeTypeEmployee, changesRead[0].DataType)
	...
	changesRead, err = l.changesClient.ChangesRead(ctx, changesdata.ChangeSearch{
		...
		Types:        []string{data.ChangeTypeEmployee},
		ServiceNames: []string{data.ServiceName},
		Actions:      []string{data.ChangeActionDelete},
	})
	assert.Nil(t, err)
	assert.NotNil(t, changesRead)
	assert.Len(t, changesRead, 1)
	assert.Equal(t, employeeUpdated.ID, changesRead[0].DataId)
	assert.Equal(t, data.ChangeActionDelete, changesRead[0].DataAction)
	assert.Equal(t, data.ServiceName, changesRead[0].DataServiceName)
	assert.Equal(t, data.ChangeTypeEmployee, changesRead[0].DataType)
}

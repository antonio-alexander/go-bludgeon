...
case "$GOOS" in
    "windows") OUTPUT_FILE="timers.exe"
    ;;
    *) OUTPUT_FILE="timers"
    ;;
esac
...
case "$1" in
    "go") go build -ldflags \
        "-X github.com/antonio-alexander/go-bludgeon/timers/cmd/internal.Version=${VERSION} \
        -X github.com/antonio-alexander/go-bludgeon/timers/cmd/internal.GitCommit=${GIT_COMMIT} \

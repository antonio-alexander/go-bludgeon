...
SELECT
    ...
    archived,
    completed,
    (SELECT uuid FROM employees WHERE id = timers.employee_id) AS employee_id,
    (SELECT uuid FROM time_slices WHERE finish IS NULL AND timer_id = timers.id) AS active_time_slice_id,
    version,
    UNIX_TIMESTAMP(last_updated) AS last_updated,
    last_updated_by
FROM 

...
func employeeRead(db interface {
	...
	case int64:
		condition = fmt.Sprintf("employee_id = (SELECT uuid FROM %s WHERE id = ?)", tableEmployees)
	}
...
func timeSliceCreate(db interface {
	...
	if timeSlicePartial.TimerID != nil {
		...
		values = append(values, fmt.Sprintf("(SELECT id FROM %s WHERE uuid = ?)", tableTimers))
	...
	query := fmt.Sprintf(`INSERT INTO %s(%s) VALUES(%s);`, tableTimeSlices, strings.Join(columns, ","), strings.Join(values, ","))
...
func timeSliceRead(db interface {
	...
	case string:
		value = "?"
	case int64:
		value = fmt.Sprintf("(SELECT uuid FROM %s WHERE id = ?)", tableTimeSlices)
	}
...
func timerRead(db interface {
	...
	case string:
		condition = "timer_id = ?"
	case int64:
		condition = fmt.Sprintf("timer_id = (SELECT uuid FROM %s WHERE id = ?)", tableTimers)
	}
